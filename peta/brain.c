...
#define STACKPRIO           (PRIORITY_MAIN - 2)
...
void brain_init(void)
{
    ...
    puts("init servo");
    servo_init(&steering, CONF_STEERING_PWM, CONF_STEERING_PWM_CHAN,
               CONF_STEERING_MIN, CONF_STEERING_MAX);
    /* initialize motor control */
    puts("init motor");
    gpio_init_out(CONF_MOTOR_DIRA, GPIO_NOPULL);
    gpio_init_out(CONF_MOTOR_DIRB, GPIO_NOPULL);
    pwm_init(CONF_MOTOR_PWM, CONF_MOTOR_PWM_CHAN,
             CONF_MOTOR_FREQ, CONF_MOTOR_RES);
