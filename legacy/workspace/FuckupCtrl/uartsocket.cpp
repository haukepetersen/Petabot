    ...
    QObject(parent)
{
    ...
    connect(uart, &QextSerialPort::readyRead, this, &UartSocket::onNewData);

	pollTimer = new QTimer(this);
	connect(pollTimer, &QTimer::timeout, this, &UartSocket::onPollEvent);
	pollTimer->start(3000);		// look every 3 seconds for a device change
	discoverdPorts = portenum.getPorts().size();

	// connect the parser
	connect(&parser, &CommandParser::commandReady, this, &UartSocket::onNewCommand);
}
